...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 25 14:38:29 2018
...
clf = KNeighborsClassifier(algorithm='auto', leaf_size=30, p=2, metric='minkowski', 
                           n_jobs=1)
# specify parameters and distributions to sample from
param_grid = {"n_neighbors": [1000, 500, 200, 100, 50, 10, 5],
              "weights": ['uniform', 'distance']}
grid_search = GridSearchCV(clf, param_grid=param_grid, cv = 5)
grid_search.fit(train_features, train_data[target])
report(grid_search.cv_results_, n_top=10)

# Complexity curve: explore the n_neighbors, when using uniform weight
n_neighbors = [1, 5, 10, 25, 50, 75, 100, 200, 500, 1000]
train_accuracy = []
cv_accuracy = []
train_precision = []
cv_precision = []
...
df_knn = pd.DataFrame({'train_accuracy': train_accuracy, \
                              'cv_accuracy': cv_accuracy,\
                              'train_precision': train_precision,\
                              'cv_precision': cv_precision, \
                              'n_neighbors': n_neighbors})
print df_knn
ax = df_knn.plot(x='n_neighbors', y=['train_accuracy','cv_accuracy','train_precision', 'cv_precision'],\
                 logx = True, title = "Complexity Curve: n-neighbors", 
                 style= '.-', ylim=(0.5, 1.0))
ax.set_xlabel("n_neighbors")
ax.set_ylabel("Accuracy/Precision")    
plt.savefig('Data1_KNN_complexityCurve.png')
...
df_loan_acc, df_loan_precison = training_size_acc(train_data, test_features, test_data[target], \
                        estimator = KNeighborsClassifier(n_neighbors= 100, weights='uniform'),\
                        scoring = ['accuracy', 'precision'])
print df_loan_acc
ax = df_loan_acc.plot(y=['train_accuracy','cv_accuracy','test_accuracy'],\
                      title = "K-NN Learning Curve: Accuracy",\
                      ylim = (0.5, 0.725), style= ".-")
ax.set_xlabel("Training size %")
ax.set_ylabel("Accuracy")  
plt.savefig('Data1_kNN_LearningCurve_acc.png')
